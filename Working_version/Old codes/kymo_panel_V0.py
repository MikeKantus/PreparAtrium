...
# compatibilidad: exponer las frames con los nombres que usan otras funciones
    ...
    def update_frame(self, idx=0):
        ...
        """
        # Obtener frames (nombre que uses en KymoPanel)
        # Selección segura de frames (evita evaluar arrays en contexto booleano)
        frames = None
        for name in ("frames", "ecc_frames", "stack"):
            candidate = getattr(self, name, None)
            if candidate is not None:
                # candidate puede ser numpy array, list, etc. lo aceptamos tal cual
                frames = candidate
                break
        ...
        try:
            ...
            # len no está definido para este tipo; asumimos que no está vacío
            pass
        if frames is None or len(frames) == 0:
            # limpiar label si existe
            for candidate in ("label_preview", "label_original", "label_kymo", "label"):
                if hasattr(self, candidate):
                    getattr(self, candidate).clear()
                    return
            return
        ...
        try:
            ...
            print("DEBUG kymo update_frame: qimg size:", qimg.size(), "frame dtype:", getattr(frame, "dtype", None))
        ...
        try:
            ...
            # Si no existen, asegurar listas vacías
            ...
            for line in lines:
                ...
                for i in range(len(line) - 1):
                    x1 = int(line[i][0] * pix.width() / frame.shape[1])
                    y1 = int(line[i][1] * pix.height() / frame.shape[0])
                    x2 = int(line[i+1][0] * pix.width() / frame.shape[1])
                    y2 = int(line[i+1][1] * pix.height() / frame.shape[0])
            ...
            if current_line and len(current_line) >= 2:
                ...
                for i in range(len(current_line) - 1):
                    x1 = int(current_line[i][0] * pix.width() / frame.shape[1])
                    y1 = int(current_line[i][1] * pix.height() / frame.shape[0])
                    x2 = int(current_line[i+1][0] * pix.width() / frame.shape[1])
                    y2 = int(current_line[i+1][1] * pix.height() / frame.shape[0])
                    painter.drawLine(x1, y1, x2, y2)
        finally:
            painter.end()
    ...
    def update_kymo_preview(self):
        if len(self.lines) == 0:
            return
        print("DEBUG update_kymo_preview: meta keys:", None if self.meta is None else list(self.meta.keys()))
        print("DEBUG update_kymo_preview: meta pixel_size:", None if self.meta is None else self.meta.get("pixel_size", None))

        last_line = self.lines[-1]
        kymo, axis_x_nm, axis_t_s = extract_kymograph(self.stack, last_line, self.meta)

        # Normalizar para mostrar
        kymo_img = (kymo - np.min(kymo)) / (np.max(kymo) - np.min(kymo)) * 255
        kymo_img = kymo_img.astype(np.uint8)

        qimg = QImage(kymo_img.data, kymo_img.shape[1], kymo_img.shape[0],
                    kymo_img.shape[1], QImage.Format_Grayscale8)
        pix = QPixmap.fromImage(qimg)
        self.label_kymo_preview.setPixmap(pix.scaled(300, 300, Qt.KeepAspectRatio))
    def finish_line(self):
        if len(self.current_line) > 1:
            self.lines.append(self.current_line.copy())
            self.current_line = []
